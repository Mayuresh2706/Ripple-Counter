...
def count_ripples_full(filtered_signal, fs=4000, dominant_freq=None, prominence=None):
    """Counts ripples across the entire signal, including big peaks, while rejecting noise."""
    ...
    # Use median absolute deviation (MAD) for robust prominence
    # This prevents the huge start/stop spikes from throwing off the threshold
    if prominence is None:
        median = np.median(filtered_signal)
        mad = np.median(np.abs(filtered_signal - median))
        # Set prominence to 2.5x the MAD (a standard statistical noise floor)
        prominence = mad * 2.5
        # Ensure a minimum absolute prominence just in case signal is perfectly flat
...
def plot_results(time, raw_signal, filtered_signal, fft_freqs, fft_mag,
                 peaks=None, dominant_freq=None, title="Motor Current Ripple Analysis"):
    """4-panel plot: Raw, FFT, Filtered (Full), Filtered (Zoomed in)."""
    ...
    # Panel 3: Filtered Signal (Full)
    ax3.plot(time, filtered_signal, color='blue', linewidth=0.5)
    if peaks is not None and len(peaks) > 0:
        # Changed marker to dot '.' so it doesn't blur into a box
        ax3.plot(time[peaks], filtered_signal[peaks], ".", color='red', markersize=4)
    ax3.axhline(0, color='black', linewidth=0.5, linestyle='--')
    ax3.set_title(f"Filtered Signal (Full) — Detected Ripples: {len(peaks) if peaks is not None else 0}")
    ax3.grid(True, alpha=0.3)

    # Panel 4: Zoomed-in View (0.5 seconds from the middle of the signal)
    mid_idx = len(time) // 2
    # Find a 0.5s window around the middle
    ...
    ax4.plot(time[start_idx:end_idx], filtered_signal[start_idx:end_idx], color='blue', linewidth=1)
    if peaks is not None and len(peaks) > 0:
        # Get peaks that fall within the zoom window
        zoom_peaks = peaks[(peaks >= start_idx) & (peaks < end_idx)]
        ax4.plot(time[zoom_peaks], filtered_signal[zoom_peaks], "o", color='red', markersize=6, fillstyle='none')
    ax4.axhline(0, color='black', linewidth=0.5, linestyle='--')
    ax4.set_title("Zoomed View (0.5s window) - Red circles show detected peaks")
    ax4.set_xlabel("Time (s)")
    ax4.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig("ripple_analysis.png", dpi=200, bbox_inches='tight')
    plt.show()
